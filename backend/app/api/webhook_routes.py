...
async def list_integrations(
    ...
    return [WebhookIntegrationResponse.model_validate(i) for i in result.scalars().all()]
...
async def create_integration(
    ...
    return WebhookIntegrationResponse.model_validate(integration)
...
async def receive_webhook(
    ...
    if not integration or not integration.is_active:
        raise HTTPException(status_code=404, detail="Webhook integration not found or inactive")

    _check_webhook_secret(integration, request)

    alert = await process_webhook_alert(db, integration_id, payload, _webhook_url(request, integration_id))
    if not alert:
